...
public class GridTimeRealsPrunedCorrectnessPlaneSweepTwoEpsilonCounter {
...
//            Grid grid = Grid.newGrid(Rectangle.newRectangle(Point.newPoint(-124.763068, 17.673976), Point.newPoint(-64.564908, 49.384359)), radius, function);
            GenericMRDSJGrid grid = GenericMRDSJGrid.newGenericGrid(Rectangle.newRectangle(Point.newPoint(-124.763068, 17.673976), Point.newPoint(-64.564909, 49.384360)), radius, 2,Integer.parseInt(args[4]));
...
//            gridBroadcasted.getValue().initialize();
            ...
            long count = pairRDD.groupByKey(new HashPartitioner(Integer.parseInt(args[3]))).map(new Function<Tuple2<Integer, Iterable<Tuple2<Integer, Tuple3<String, Double, Double>>>>,Long>() {
                @Override
                public Long call(Tuple2<Integer, Iterable<Tuple2<Integer, Tuple3<String, Double, Double>>>> tup) throws Exception {
                    ...
                    counter = counter + Algorithms.planeSweepSelf(homeObjects, radius);

                    for (int i = 0; i < replicatedObjectsList.size(); i++) {
                        counter = counter + Algorithms.planeSweep(homeObjects, replicatedObjectsList.get(i), radius);
                    }
                    ...
                    for (Tuple3<String, Double, Double>[] tuple3s : replicatedObjectsList) {
                        if(tuple3s.length>0){
                            ...
                            if(id==(tup._1-(int)gridBroadcasted.getValue().getCellsInXAxis()) || id==(tup._1.intValue()+(int)gridBroadcasted.getValue().getCellsInXAxis())){
                                list2 = tuple3s;
                            }
                        }
                    }
                    ...
                    if(list1!=null && list2!=null){
                        counter = counter + Algorithms.planeSweep(list1, list2, radius);
                    }
...
//                        if(replicatedObject.getKey() == (tup._1-1) || replicatedObject.getKey() == (tup._1+1)){
//                            list1 = replicatedObject;
//                        }
//                        if(replicatedObject.getKey()==(tup._1-gridBroadcasted.getValue().getCellsInXAxis()) || replicatedObject.getKey()==(tup._1+gridBroadcasted.getValue().getCellsInXAxis())){
//                            list2 = replicatedObject;
//                        }
//                    }
//
//                    if(list1!=null && list2!=null){
//                        planeSweep1(list1.getValue(), list2.getValue(), results, radius);
//                    }
        ...
        FileWriter fw = new FileWriter("./timeExec-"+args[2]+"-"+args[1]+"-mrdsj-2e-ps.txt", true);
