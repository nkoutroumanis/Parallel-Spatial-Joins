...
public class GridTimeRealsPrunedCorrectnessPlaneSweepXCounter {
...
//            gridBroadcasted.getValue().initialize();
            ...
            long count = pairRDD.groupByKey(cp).map(new Function<Tuple2<Integer, Iterable<Tuple3<String, Double, Double>>>,Long>() {
                @Override
                public Long call(Tuple2<Integer, Iterable< Tuple3<String, Double, Double>>> tup) throws Exception {
                    ...
                    counter = counter + Algorithms.planeSweepSelfOnY(homeObjects, radius);

                    for (int i = 0; i < replicatedObjectsList.size(); i++) {
                        counter = counter + Algorithms.planeSweepOnY(homeObjects, replicatedObjectsList.get(i), radius);
                    }
