...
public class SelfGrid {
    ...
    private Function4<Cell, Cell, Point, Edge> function;

//    private final boolean geoData;

//    private final Function4<Cell, Double, Double, Double, Position> getPositionFunction;

    private SelfGrid(Rectangle rectangle, int cellsInXAxis, int cellsInYAxis, double r, /*ReplicationType function*/Function4<Cell, Cell, Point, Edge> function, boolean geoData){
...
//         });
    ...
    public void printInfoAgreements(){
...
//    }
    ...
    public void addPointDataset(double x, double y){
        ...
        cell.addPointToDataset(cell.getPosition(x,y,r));//getPositionFunction.apply(cell,x,y,r));
...
//    }
    ...
    private void getPartitions(int cellID, Agreements agreements, Position position, List<Integer> partitions){
        switch(position){
            case TOP:
                agreements.getPartitionsTwoSpacesCase((cellID + cellsInXAxis), partitions);
                break;
            case BOTTOM:
                agreements.getPartitionsTwoSpacesCase((cellID - cellsInXAxis), partitions);
                break;
            case RIGHT:
                agreements.getPartitionsTwoSpacesCase((cellID  + 1), partitions);
                break;
            case LEFT:
                agreements.getPartitionsTwoSpacesCase((cellID  - 1), partitions);
                break;
...
//    }
    ...
    private void getPartitions(int cellId, Agreements agreements, Position position, boolean outerQuarter, List<Integer> partitions){
        switch (position){
            case TOPRIGHT:
//                if(agreements.getSpacesNum()==4){
                agreements.getPartitionsFourSpacesCase(0, outerQuarter, partitions);
//                }else if(hasTop(cellId)){
//                    getPartitions(cellId, agreements, TOP, typeSet, partitions);
//                }else if(hasRight(cellId)){
//                    getPartitions(cellId, agreements, RIGHT, typeSet, partitions);
//                }
            ...
            case TOPLEFT:
//                if(agreements.getSpacesNum()==4){
                agreements.getPartitionsFourSpacesCase(1, outerQuarter, partitions);
//                }else if(hasTop(cellId)){
//                    getPartitions(cellId, agreements, TOP, typeSet, partitions);
//                }else if(hasLeft(cellId)){
//                    getPartitions(cellId, agreements, LEFT, typeSet, partitions);
//                }
            ...
            case BOTTOMRIGHT:
//                if(agreements.getSpacesNum()==4){
                agreements.getPartitionsFourSpacesCase(2, outerQuarter, partitions);
//                }else if(hasBottom(cellId)){
//                    getPartitions(cellId, agreements, BOTTOM, typeSet, partitions);
//                }else if(hasRight(cellId)){
//                    getPartitions(cellId, agreements, RIGHT, typeSet, partitions);
//                }
            ...
            case BOTTOMLEFT:
//                if(agreements.getSpacesNum()==4){
                agreements.getPartitionsFourSpacesCase(3, outerQuarter, partitions);
//                }else if(hasBottom(cellId)){
//                    getPartitions(cellId, agreements, BOTTOM, typeSet, partitions);
//                }else if(hasLeft(cellId)){
//                    getPartitions(cellId, agreements, LEFT, typeSet, partitions);
...
//    }
    ...
    public static SelfGrid newGrid(Rectangle rectangle, int cellsInXAxis, int cellsInYAxis, double r, /*ReplicationType function*/  Function4<Cell, Cell, Point, Edge> function){
    ...
    public static SelfGrid newGrid(Rectangle rectangle, double r, Function4<Cell, Cell, Point, Edge> function){
    ...
    public static SelfGrid newGridStripesX(Rectangle rectangle, double r, Function4<Cell, Cell, Point, Edge> function){
        double cellsInXAxis = ((rectangle.getUpperBound().getX() - rectangle.getLowerBound().getX())/(2*r));
        if(cellsInXAxis%1 ==0){
            cellsInXAxis = cellsInXAxis - 1;
        }
    ...
    public static SelfGrid newGridStripesY(Rectangle rectangle, double r, Function4<Cell, Cell, Point, Edge> function){
    ...
    public static SelfGrid newGeoGrid(Rectangle rectangle, int cellsInXAxis, int cellsInYAxis, double r, /*ReplicationType function*/Function4<Cell, Cell, Point, Edge> function){
        System.out.println("Cells in X axis:"+ cellsInXAxis);
        System.out.println("Cells in Y axis:"+ cellsInYAxis);
        return new SelfGrid(rectangle, cellsInXAxis, cellsInYAxis, r, function, true);
    }

    public static SelfGrid newGeoGrid(Rectangle rectangle, double r, /*ReplicationType function*/Function4<Cell, Cell, Point, Edge> function){
...
//        List<Integer> cellsIds = cells.entrySet().stream().sorted(Comparator.comparing(c->c.getValue().getNumberOfPointsType(), Comparator.reverseOrder())).limit(10000).map(g->g.getKey()).collect(Collectors.toList());
...
//                    Edge e = getAgreements(cellID, cell.getRectangle().getLowerBound(),0).getEdge(5);
                    if (!e.isEliminated()) {
                        if (e.getDirectionCode()) {
                            extraA = extraA + cellN.getNumberOfPointsInTopRightQuarterArea();
                        }
                    }
                }
...
//                    Edge e = getAgreements(cellID, cell.getRectangle().getLowerRightBound(),1).getEdge(7);
                    if (!e.isEliminated()) {
                        if (e.getDirectionCode()) {
                            extraA = extraA + cellN.getNumberOfPointsInTopLeftQuarterArea();
                        }
                    }
                }
...
//                    Edge e = getAgreements(cellID, cell.getRectangle().getUpperLeftBound(),2).getEdge(8);
                    if (!e.isEliminated()) {
                        if (!e.getDirectionCode()) {
                            extraA = extraA + cellN.getNumberOfPointsInBottomRightQuarterArea();
                        }
                    }
                }
...
//                    Edge e = getAgreements(cellID, cell.getRectangle().getUpperBound(),3).getEdge(6);
                    if (!e.isEliminated()) {
                        if (!e.getDirectionCode()) {
                            extraA = extraA + cellN.getNumberOfPointsInBottomLeftQuarterArea();
                        }
                    }
                }
...
//                    Edge e1 = getAgreements(cellID, cell.getRectangle().getLowerRightBound(),1).getEdge(hasRight(cellID)?3:1);
                    if (e.getDirectionCode()) {
                        extraA = extraA + cellN.getNumberOfPointsInTopSpecialArea();
                        if (e.isEliminated()) {
                            extraA = extraA - cellN.getNumberOfPointsInTopLeftQuarterArea();
                        }
                        if (e1.isEliminated()) {
...
//                    Edge e1 = getAgreements(cellID, cell.getRectangle().getUpperLeftBound(),2).getEdge(hasTop(cellID)?1:1);
                    if (e.getDirectionCode()) {
                        extraA = extraA + cellN.getNumberOfPointsInRightSpecialArea();
                        if (e.isEliminated()) {
                            extraA = extraA - cellN.getNumberOfPointsInBottomRightQuarterArea();
                        }
                        if (e1.isEliminated()) {
...
//                    Edge e1 = getAgreements(cellID, cell.getRectangle().getLowerRightBound(),1).getEdge(hasBottom(cellID)?12:2);
                    ...
                    if (!e.getDirectionCode()) {
                        extraA = extraA + cellN.getNumberOfPointsInLeftSpecialArea();
                        if (e.isEliminated()) {
                            extraA = extraA - cellN.getNumberOfPointsInTopLeftQuarterArea();
                        }
                        if (e1.isEliminated()) {
...
//                    Edge e1 = getAgreements(cellID, cell.getRectangle().getUpperLeftBound(),2).getEdge(hasLeft(cellID)?10:2);
                    if (!e.getDirectionCode()) {
                        extraA = extraA + cellN.getNumberOfPointsInBottomSpecialArea();
                        if (e.isEliminated()) {
                            extraA = extraA - cellN.getNumberOfPointsInBottomRightQuarterArea();
                        }
                        if (e1.isEliminated()) {
                ...
                if ((cell.getNumberOfPoints() + extraA) != 0) {
//                    int k = ((cell.getNumberOfPoints())*((cell.getNumberOfPoints())-1)/2) + cell.getNumberOfPoints()*extraA;//(cell.getNumberOfPointsType() + extraA)*((cell.getNumberOfPointsType() + extraA)-1)/2 ;
                    int k = ((cell.getNumberOfPoints())*(cell.getNumberOfPoints())) + cell.getNumberOfPoints()*extraA;//(cell.getNumberOfPointsType() + extraA)*((cell.getNumberOfPointsType() + extraA)-1)/2 ;
...
//        Map<Integer, Edge[]> aggrLocalMap = new HashMap<>();
//        System.out.println("Aggrements size was"+agreements.size());
        Iterator o1 = agreements.entrySet().iterator();
        while (o1.hasNext()) {
            Map.Entry<Integer, Agreements> e = (Map.Entry<Integer, Agreements>) o1.next();
...
//            else{
//                    if (aggrLocalMap.containsKey(Arrays.hashCode(e.getValue().getEdges()))) {
//                        agreements.get(e.getKey()).setEdges(aggrLocalMap.get(Arrays.hashCode(e.getValue().getEdges())));
//                    } else {
//                        aggrLocalMap.put(Arrays.hashCode(e.getValue().getEdges()), e.getValue().getEdges());
//                    }
//            }
        }
//        System.out.println("aggrLocalMap size is "+aggrLocalMap.size());
//        aggrLocalMap.clear();
...
//                }
    ...
    public int getPartitionsInExecutorHome(double x, double y){
        int cellID = getCellId(x,y);
        Cell cell = getCellInExecutor(cellID);
        return getPartitionId(cell);
    }

    public int[] getPartitionsInExecutor(double x, double y){
        int cellID = getCellId(x,y);
        Cell cell = getCellInExecutor(cellID);
            ...
            case TOPRIGHTQUARTER:
                if(!existsInAgreements(getAggID(cellID, 3))){
                    ...
                    if(aggr.getSpacesNum()==4){
                        getPartitions(cellID, aggr, TOPRIGHT, false, partitions);
                    }else if(hasTop(cellID)){
                        getPartitions(cellID, aggr, TOP, partitions);
                    }else if(hasRight(cellID)){
                        getPartitions(cellID, aggr, RIGHT, partitions);
                    }
                }
//                checkForExtraPartitionSuppArea(cell, cellID, TOPLEFT, true, x,y, partitions);
//                checkForExtraPartitionSuppArea(cell, cellID, BOTTOMRIGHT, false, x,y, partitions);
                break;
            case TOPLEFTQUARTER:
                if(!existsInAgreements(getAggID(cellID, 2))){
                    ...
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, TOPLEFT, false, partitions);
                    }else if(hasTop(cellID)){
                        getPartitions(cellID, aggr, TOP, partitions);
                    }else if(hasLeft(cellID)){
                        getPartitions(cellID, aggr, LEFT, partitions);
                    }
                }
//                checkForExtraPartitionSuppArea(cell, cellID, TOPRIGHT, true, x, y, partitions);
//                checkForExtraPartitionSuppArea(cell, cellID, BOTTOMLEFT, false, x, y, partitions);
                break;
            case BOTTOMRIGHTQUARTER:
                if(!existsInAgreements(getAggID(cellID, 1))){
                    ...
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, BOTTOMRIGHT, false, partitions);
                    } else if(hasBottom(cellID)){
                        getPartitions(cellID, aggr, BOTTOM, partitions);
                    }else if(hasRight(cellID)){
                        getPartitions(cellID, aggr, RIGHT, partitions);
                    }
                }
//                checkForExtraPartitionSuppArea(cell, cellID, BOTTOMLEFT, true, x, y, partitions);
//                checkForExtraPartitionSuppArea(cell, cellID, TOPRIGHT, false, x, y, partitions);
                break;
            case BOTTOMLEFTQUARTER:
                if(!existsInAgreements(getAggID(cellID, 0))){
                    ...
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, BOTTOMLEFT, false, partitions);
                    }else if(hasBottom(cellID)){
                        getPartitions(cellID, aggr, BOTTOM, partitions);
                    }else if(hasLeft(cellID)){
                        getPartitions(cellID, aggr, LEFT, partitions);
...
//                checkForExtraPartitionSuppArea(cell, cellID, BOTTOMRIGHT, true, x, y, partitions);
//                checkForExtraPartitionSuppArea(cell, cellID, TOPLEFT, false, x, y, partitions);
                break;
            case TOPRIGHT:
                if(!existsInAgreements(getAggID(cellID, 3))){
                    ...
                    aggr = getAgreementsInExecutor(getAggID(cellID, 3));
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, TOPRIGHT, true, partitions);
                    }else if(hasTop(cellID)){
                        getPartitions(cellID, aggr, TOP, partitions);
                    }else if(hasRight(cellID)){
                        getPartitions(cellID, aggr, RIGHT, partitions);
...
//                checkForExtraPartition(cell, cellID, TOPLEFT, true, x, y, partitions);
//                checkForExtraPartition(cell, cellID, BOTTOMRIGHT, false, x, y, partitions);
                break;
            case TOPLEFT:
                if(!existsInAgreements(getAggID(cellID, 2))){
                    ...
                    aggr = getAgreementsInExecutor(getAggID(cellID, 2));
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, TOPLEFT, true, partitions);
                    }else if(hasTop(cellID)){
                        getPartitions(cellID, aggr, TOP, partitions);
                    }else if(hasLeft(cellID)){
                        getPartitions(cellID, aggr, LEFT, partitions);
...
//                checkForExtraPartition(cell, cellID, TOPRIGHT, true, x, y, partitions);
//                checkForExtraPartition(cell, cellID, BOTTOMLEFT, false, x, y, partitions);
                break;
            case BOTTOMRIGHT:
                if(!existsInAgreements(getAggID(cellID, 1))){
                    ...
                    aggr = getAgreementsInExecutor(getAggID(cellID, 1));
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, BOTTOMRIGHT, true, partitions);
                    }else if(hasBottom(cellID)){
                        getPartitions(cellID, aggr, BOTTOM, partitions);
                    }else if(hasRight(cellID)){
                        getPartitions(cellID, aggr, RIGHT, partitions);
...
//                checkForExtraPartition(cell, cellID, BOTTOMLEFT, true, x, y, partitions);
//                checkForExtraPartition(cell, cellID, TOPRIGHT, false, x, y, partitions);
                break;
            case BOTTOMLEFT:
                if(!existsInAgreements(getAggID(cellID, 0))){
                    ...
                    aggr = getAgreementsInExecutor(getAggID(cellID, 0));
                    if(aggr.getSpacesNum()==4) {
                        getPartitions(cellID, aggr, BOTTOMLEFT, true, partitions);
                    }else if(hasBottom(cellID)){
                        getPartitions(cellID, aggr, BOTTOM, partitions);
                    }else if(hasLeft(cellID)){
                        getPartitions(cellID, aggr, LEFT, partitions);
...
//                checkForExtraPartition(cell, cellID, BOTTOMRIGHT, true, x, y, partitions);
//                checkForExtraPartition(cell, cellID, TOPLEFT, false, x, y, partitions);
                break;
            case TOP:
                if(hasTop(cellID)){
                    if(!existsInAgreements(getAggID(cellID, 3))){
                        ...
                        aggr = getAgreementsInExecutor(getAggID(cellID, 3));
                        aggr.getPartitionsNEW(0, 2, true, partitions);
                    }
                }
//                checkForExtraPartition(cell, cellID, TOPLEFT, true, x,y,partitions);
//                checkForExtraPartition(cell, cellID, TOPRIGHT, true, x,y,partitions);
            ...
            case BOTTOM:
                if(hasBottom(cellID)){
                    if(existsInAgreements(getAggID(cellID, 0))){
                        aggr = getAgreementsInExecutor(getAggID(cellID, 0));
                        aggr.getPartitionsNEW(3, 1, false, partitions);
                    }
                }
//                checkForExtraPartition(cell, cellID, BOTTOMLEFT, true, x,y, partitions);
//                checkForExtraPartition(cell, cellID, BOTTOMRIGHT, true, x,y, partitions);
            ...
            case RIGHT:
                if(hasRight(cellID)){
                    if(!existsInAgreements(getAggID(cellID, 3))){
                        ...
                        aggr = getAgreementsInExecutor(getAggID(cellID, 3));
                        aggr.getPartitionsNEW(0, 1, true, partitions);
                    }
                }
//                checkForExtraPartition(cell, cellID, TOPRIGHT, false, x, y, partitions);
//                checkForExtraPartition(cell, cellID, BOTTOMRIGHT, false, x, y, partitions);
            ...
            case LEFT:
                if(hasLeft(cellID)){
                    if(existsInAgreements(getAggID(cellID, 0))){
                        aggr = getAgreementsInExecutor(getAggID(cellID, 0));
                        aggr.getPartitionsNEW(3, 2, false, partitions);
                    }
                }
//                checkForExtraPartition(cell, cellID, TOPLEFT, false, x, y, partitions);
//                checkForExtraPartition(cell, cellID, BOTTOMLEFT, false, x, y, partitions);
...
//        if(Double.compare(-85.8345425 ,x)==0 && Double.compare(34.8750619,y)==0){
//        checkForExtraPartitionSuppArea(cell, cellID, BOTTOMLEFT, true, x, y, partitions);
////        checkForExtraPartitionSuppArea(cell, cellID, TOPRIGHT, false, x, y, partitions);
//        }
...
//                if(partitions.get(i).equals(partitions.get(j))){
//                    try {
//                        throw new Exception("Same partitions in list "+ cell.getPosition(x,y,r).toString()+" "+x+" "+y+" "+partitions+"\n"+getAgreementsInExecutor(getAggID(cellID, 0)).toString()+"\n"+ getAgreementsInExecutor(getAggID(cellID, 1)));
//                    } catch (Exception e) {
//                        throw new RuntimeException(e);
//                    }
//                }
...
//    }
    ...
    private void checkForExtraPartitionSuppArea(Cell cell, int cellID, Position position, boolean horizontal1, double x, double y, List<Integer> list) {
//        if(!geoData){
        checkForExtraPartition(cell, cellID, position, horizontal1, x, y, list);
//        }else{
//            //checkForExtraGeoPartition(cell, cellId, position, horizontal1, point, typeSet, list);
//        }
    }

    private void checkForExtraPartition(Cell cell, int cellID, Position position, boolean horizontal, double x, double y, List<Integer> partitions) {
        int extraCell;
        Agreements aggr = null;
        switch (position){
            case TOPRIGHT:
                ...
                if(aggr!=null) {
                    if (aggr.getSpacesNum() == 4) {
                        if (horizontal) {
                            extraCell = aggr.getExtraPartition(2,false,1,true,3,true);
                        } else {
                            extraCell = aggr.getExtraPartition(1,false,2,true,3,true);
                        }
//                    Optional<Cell> extraCell = getAgreements(cellId, cell.getRectangle().getUpperBound()).getExtraPartition(cell,(horizontal)?(getCell((cellId + cellsInXAxis))):(getCell((cellId  + 1))), typeSet);
            ...
            case TOPLEFT:
                ...
                if(aggr!=null) {
                    if (aggr.getSpacesNum() == 4) {
                        if (horizontal) {
                            extraCell = aggr.getExtraPartition(5,false,1,false,4,true);
                        } else {
                            extraCell = aggr.getExtraPartition(1,true,5,true,4,true);
                        }
//                    Optional<Cell> extraCell = getAgreements(cellId, cell.getRectangle().getUpperLeftBound()).getExtraPartition(cell,(horizontal)?(getCell((cellId + cellsInXAxis))):(getCell((cellId  - 1))), typeSet);
            ...
            case BOTTOMRIGHT:
                ...
                if(aggr!=null) {
                    ...
                    if (aggr.getSpacesNum() == 4) {
                        if (horizontal) {
                            extraCell = aggr.getExtraPartition(2,true,6,true,4,false);
                        } else {
                            extraCell = aggr.getExtraPartition(6,false,2,false,4,false);
                        }
//                    Optional<Cell> extraCell = getAgreements(cellId, cell.getRectangle().getLowerRightBound()).getExtraPartition(cell,(horizontal)?(getCell((cellId - cellsInXAxis))):(getCell((cellId  + 1))), typeSet);
            ...
            case BOTTOMLEFT:
                ...
                if(aggr!=null) {
                    ...
                    if (aggr.getSpacesNum() == 4) {
                        if (horizontal) {
                            extraCell = aggr.getExtraPartition(5,true,6,false,3,false);
                        } else {
                            extraCell = aggr.getExtraPartition(6,true,5,false,3,false);
                        }
