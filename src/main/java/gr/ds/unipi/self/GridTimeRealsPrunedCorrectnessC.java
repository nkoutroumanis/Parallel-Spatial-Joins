...
public class GridTimeRealsPrunedCorrectnessC {
...
//            gridBroadcasted.getValue().initialize();
            ...
            JavaPairRDD<Integer, Tuple3<String, Double, Double>> pairRDDA = rddTuplesA.mapToPair(new PairFunction<Tuple3<String, Double, Double>, Integer, Tuple3<String, Double, Double>>() {
                @Override
                public Tuple2<Integer, Tuple3<String, Double, Double>> call(Tuple3<String, Double, Double> tuple) throws Exception {
                    int cellId = gridBroadcasted.getValue().getPartitionsInExecutorHome(tuple._2(), tuple._3());
                    return new Tuple2<>(cellId, tuple);
                }
            });

            JavaPairRDD<Integer, Tuple2<Integer,Tuple3<String, Double, Double>>> pairRDDB = rddTuplesA.flatMapToPair(new PairFlatMapFunction<Tuple3<String, Double, Double>, Integer, Tuple2<Integer,Tuple3<String, Double, Double>>>() {
                @Override
            ...
            long count = pairRDDA.cogroup(pairRDDB,cp).flatMap(new FlatMapFunction<Tuple2<Integer, Tuple2<Iterable<Tuple3<String, Double, Double>>, Iterable<Tuple2<Integer,Tuple3<String, Double, Double>>>>>, Tuple2<Tuple3<String, Double, Double>,Tuple3<String, Double, Double>>>() {
                @Override
                public Iterator<Tuple2<Tuple3<String, Double, Double>, Tuple3<String, Double, Double>>> call(Tuple2<Integer, Tuple2<Iterable<Tuple3<String, Double, Double>>, Iterable<Tuple2<Integer, Tuple3<String, Double, Double>>>>> tup) throws Exception {
                    ...
                    for (int i = 0; i < replicatedObjects.size(); i++) {
                        for (int j = i + 1; j < replicatedObjects.size(); j++) {
                            if(Math.abs(replicatedObjects.get(i).getKey()-replicatedObjects.get(j).getKey())==1 ||
                                ...
                                bruteForce1(replicatedObjects.get(i).getValue(), replicatedObjects.get(j).getValue(), results, radius);
                            }
                        }
                    }
    ...
    private static void bruteForce(List<Tuple3<String, Double, Double>> list1, List<Tuple3<String, Double, Double>> list2, List<Tuple2<Tuple3<String,Double,Double>, Tuple3<String,Double,Double>>> results, double radius){
        for (int i = 0; i < list1.size()-1; i++) {
            for (int j = i+1; j < list2.size(); j++) {
                if((Math.pow((list1.get(i)._2() - list2.get(j)._2()), 2) + Math.pow((list1.get(i)._3()- list2.get(j)._3()), 2) <= Math.pow(radius, 2))){
                    results.add(Tuple2.apply(list1.get(i), list2.get(j)));
                }
            }
        }
    }

    private static void bruteForce1(List<Tuple3<String, Double, Double>> list1, List<Tuple3<String, Double, Double>> list2, List<Tuple2<Tuple3<String,Double,Double>, Tuple3<String,Double,Double>>> results, double radius){
