...
public class MetricsRealsPruned {
    ...
    public static void main(String args[]) throws IOException {
        ...
        BufferedWriter w = new BufferedWriter(new FileWriter("./metrics-"+args[2] +"-"+args[3]+"-"+args[1]+"-"+"goaStar"+".txt"));
        ...
        SparkConf sparkConf = new SparkConf().set("spark.serializer","org.apache.spark.serializer.KryoSerializer").set("spark.kryo.registrationRequired", "true")
                .registerKryoClasses(new Class[]{gr.ds.unipi.grid.Grid.class, Cell.class, Grid.class, Position.class, TypeSet.class, ReplicationFunction.class, java.util.HashMap.class, Point.class, Rectangle.class, ArrayList.class, java.lang.invoke.SerializedLambda.class, org.apache.spark.util.collection.CompactBuffer[].class, org.apache.spark.util.collection.CompactBuffer.class, scala.reflect.ManifestFactory$.class,scala.reflect.ManifestFactory$.MODULE$.Any().getClass(),  UnsafeHashedRelation.class,org.apache.spark.sql.execution.columnar.DefaultCachedBatch.class, byte[][].class, org.apache.spark.sql.catalyst.expressions.GenericInternalRow.class, org.apache.spark.unsafe.types.UTF8String.class
                });
        ...
        Dataset<Row> df2 = sparkSession.read().option("delimiter", "\t").csv(path +args[3]+".csv").withColumnRenamed("_c0", "id_2").withColumnRenamed("_c1", "x_2").withColumnRenamed("_c2", "y_2");
            ...
            JavaRDD<String> rddTxtFilesB = jsc.textFile(path +args[3]+".csv");
            ...
            sampleTuples = rddTuplesA.sample(false, Double.parseDouble(args[5]),261).collect();
            ...
            sampleTuples = rddTuplesB.sample(false, Double.parseDouble(args[5]),261).collect();
        ...
        UserDefinedFunction udfCoordinatesToArrayA = udf(
                (String x, String y) -> gridBroadcasted.getValue().getPartitionsAType(Double.parseDouble(x), Double.parseDouble(y)), DataTypes.createArrayType(DataTypes.IntegerType)
        );

        UserDefinedFunction udfCoordinatesToArrayB = udf(
                (String x, String y) -> gridBroadcasted.getValue().getPartitionsBType(Double.parseDouble(x), Double.parseDouble(y)), DataTypes.createArrayType(DataTypes.IntegerType)
        );
...
//        long q = df2.withColumn("partitions", functions$.MODULE$.explode(functions$.MODULE$.col("partitions"))).groupBy("partitions").agg(functions$.MODULE$.count("partitions")/*.as("btype")*/).count();
//        System.out.println("Printing q:"+ q);

        df2 = df2.withColumn("partitions", functions$.MODULE$.explode(functions$.MODULE$.col("partitions"))).groupBy("partitions").agg(functions$.MODULE$.count("partitions").as("btype"));
