...
public class GridRunSkewed3 {

    @Test
    public void newGrid() throws IOException {
        SparkSession sparkSession = SparkSession.builder().master("local[*]").getOrCreate();

        String path = "/Users/nicholaskoutroumanis/Desktop/joins/largeSets/";
        double radius = 0.5;
        ...
        for (Map.Entry<String, Map.Entry<NewFunc, String>> entry : s) {
            ...
            String line = null;
                ...
                long replication = 0;
                Dataset<Row> df1 = sparkSession.read().option("delimiter", ";").csv(entry.getKey()+"datasetA.csv").withColumnRenamed("_c0", "x").withColumnRenamed("_c1", "y")
                ...
                Dataset<Row> df2 = sparkSession.read().option("delimiter", ";").csv(entry.getKey()+"datasetB.csv").withColumnRenamed("_c0", "x").withColumnRenamed("_c1", "y")
                        .withColumn("partitions", functions$.MODULE$.regexp_replace(functions$.MODULE$.col("_c2"), "\\[", "")).withColumn("partitions", functions$.MODULE$.regexp_replace(functions$.MODULE$.col("partitions"), "\\]", ""))
                        .drop("_c2").withColumn("partitions", functions$.MODULE$.split(functions$.MODULE$.col("partitions"), ", "));
